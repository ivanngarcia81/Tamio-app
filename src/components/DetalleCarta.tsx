 ...
 *
 * ⚠️ **HOY NO LO RENDERIZA NADIE.** Vino de la rama
 * `claude/design-review-execution-can5gv` en la fusión del 22 ago 2026, donde
 * su `Cartas.tsx` lo montaba. Ganó la otra `Cartas.tsx` —la que además lleva
 * los traslados de entrada y salida en la misma pantalla— y esa pinta el
 * detalle en línea, no con este componente.
 *
 * Se conserva porque su idea es mejor que la que quedó: enseñar la carta
 * REAL, la misma que sale por la impresora, en vez de una segunda maqueta de
 * los mismos campos. Cablearlo es su propia tarea: `Cartas.tsx` tiene un
 * modelo de pestañas (resumen / salida / archivo) que este componente no
 * conoce, y meterlo a la fuerza dentro de una fusión de dos ramas era pedir
 * un fallo. Su CSS (`.dm-carta-marco`, `.dm-carta-doc`, `.dm-doc`) sigue en
 * `styles.css`, así que el día que se cablee no hay que reescribirlo.
 *
