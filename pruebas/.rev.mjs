...
await page.evaluate(async () => {
  const q = "INSERT INTO usuarios (church_id, nombre, rol, email) VALUES (1, $1, $2, $3)";
  for (const [n, r, e] of [
    ["Iván García", "tesorero", "ivan@miiglesia.mx"],
    ["Rosa Elena Vega", "asistente", null],
  ]) await window.__TAURI_INTERNALS__.invoke("db_execute", { query: q, params: [n, r, e] });
});

async function zona(nombre, slug) {
  /* `reload`: navegar al mismo hash no cierra la zona abierta, y el índice
     se queda oculto detrás de ella. */
  await page.goto(`${URL_BASE}/#/configuracion`, { waitUntil: "networkidle" });
  await page.reload({ waitUntil: "networkidle" });
  await page.waitForTimeout(900);
  await page.locator(".ios-lista .ios-row", { hasText: new RegExp(`^${nombre}`) }).first().click();
  await page.waitForTimeout(1200);
  await page.screenshot({ path: `${SALIDA}/64-${slug}.png`, fullPage: true });
  console.log(`  ✓ 64-${slug}.png`);
}
await zona("Institución", "institucion");
await zona("Cuenta", "cuenta");
await zona("Acceso y áreas", "acceso");

/* Y la ficha de una persona, que es lo que sustituye a la tarjeta de escritorio. */
await page.getByText("Añadir persona", { exact: true }).click();
await page.waitForTimeout(1100);
await page.screenshot({ path: `${SALIDA}/64-usuario-hoja.png` });
console.log("  ✓ 64-usuario-hoja.png");
