...
| **28 ago — SUBIDA, confirmada por Iván** | **1.3.2 — el iPhone en su propio idioma.** Veintidós commits de la rama del móvil, y no es más pulido: es el vocabulario nativo del teléfono. La **cabecera de marca** (banda verde, rueda y nombre del área); **cuatro pantallas que dejan de ser una ventana de Mac encogida** —el detalle de una actividad, la ficha del miembro en tres pasos con detentes de verdad, el corte y su depósito abiertos desde el teléfono, y tocar un movimiento que por fin lo abre—; **Inicio rehecho tres veces** hasta las cuatro cifras en tarjeta; **Secretaría y Tesorería en el sistema nuevo** (actas por año, bitácora por mes, calendario, la cifra del periodo); **Por revisar** con sus siete alertas en dos grupos; **Cartas** con las cinco pestañas. Más 2126 líneas de CSS y `pruebas/hoja-contactos.mjs`, que junta las 38 capturas en UNA imagen —una pantalla enseña si ESA está bien; las 38 juntas enseñan si son la misma app—. Arnés: 1105 ✓ / 0 ✗; tipografía en **563** tamaños atados a la escala. **Y aquí la regla de contar antes de tocar cazó su primera colisión de verdad**: `1.3.1` salía DOS veces en `Cargo.lock`, la 3852 de Tamio y la 2708 de `proc-macro-crate`, que va en esa misma versión. Un buscar-y-reemplazar habría corrompido la dependencia |
| **28 ago — SUBIDA, confirmada por Iván** | **1.3.3 — aprobar desde el teléfono, y Ajustes en iOS.** Cierra **el hueco funcional más viejo que quedaba**: el iPhone era la única plataforma donde aprobar un movimiento NO existía —la fila de la Bandeja llevaba al editor y había que guardar desde dentro, cuatro toques y una pantalla entera para no cambiar nada—. Ahora es un toque en la propia fila, y sólo en las alertas `pendiente`: en las demás el movimiento YA está aprobado y el botón no cambiaría nada visible, que es peor que no tenerlo. Más **Ajustes entero en iOS** (las ocho zonas, la lámina de filas, Preferencias, Iglesia con el logo visible a 44 px, la zona de acceso por persona y por área), **editar recurrente** —el último formulario crudo del teléfono—, **el editor de plantillas de carta**, y **dos reglas que MIDEN en vez de mirar**: `medir-margenes.mjs` («¿empieza todo en la misma línea?») y `medir-ritmo.mjs` (el aire vertical), de las que salieron cinco arreglos que sólo se veían en la foto. Arnés: 1105 ✓ / 0 ✗; tipografía en **598** tamaños. **Segunda colisión seguida en `Cargo.lock`**: `1.3.2` salía dos veces, la 3852 de Tamio y la **257 de `bitflags`**. La anterior fue `proc-macro-crate` en 1.3.1. Ya no es casualidad: en un lockfile de 3900 líneas, cualquier número de versión corriente choca con algún paquete |
| **28 ago — SUBIDA, confirmada por Iván** | **1.3.4 — la zona sensible deja de tratar igual lo que borra y lo que exporta.** Era una lista de iguales: cuatro filas del mismo alto, y la única diferencia entre «exportar un CSV» y «borrarlo todo» era el color de un icono de 24 px. Un color no frena a nadie. Ahora cada acción irreversible es **su propia tarjeta con contorno rojo**, con la consecuencia contada **en cifras reales leídas de esta base** —no inventadas—, y un botón que termina en «Continuar…»: no borra, abre la pantalla que pregunta. Y esa pantalla pide **el NOMBRE de la iglesia**, no la palabra «BORRAR» —que estaría escrita en la misma pantalla y se copiaría sin leer—; el botón nace apagado y sólo se enciende cuando el nombre coincide. Más **la ficha del aportante en el teléfono** (T9, T10, T11) con su resumen por año, y **el alta de aportante**, la última pestaña del «+» que no tenía hoja. Arnés: **1106** ✓ / 0 ✗ —una comprobación MÁS que la versión anterior—, y `verificar-borrado` extendido para vigilar también la lista de lo que se CUENTA antes de borrar. **Tercera colisión seguida, y la primera en `package-lock.json`**: `1.3.3` salía tres veces, dos de Tamio y una de `tiny-invariant`. Antes fueron `proc-macro-crate` y `bitflags`, las dos en `Cargo.lock`. Los DOS lockfiles tienen la trampa |
| **29 ago — SUBIDA, confirmada por Iván** | **1.3.5 — la puerta del teléfono y el primer arranque.** Las dos pantallas que se ven **antes de que exista la app** eran las últimas que seguían heredando la maqueta de escritorio: una tarjeta de formulario flotando sobre un telón oscuro, que dentro de 393 px es una caja dentro de otra caja. Ahora el primer arranque va sobre el **verde de marca a pantalla completa** —la primera pantalla de Tamio es Tamio— con la tarjeta de campos en blanco sólido, porque sobre color un campo translúcido es lo primero que se pierde de vista y es justo donde hay que teclear. Y la puerta es lo que iOS pone en sus pantallas de cuenta: lista agrupada inset, botón de 50 que se enciende al escribir, y **la contraseña con ojo** —escribir doce caracteres a ciegas en un teclado de teléfono es el motivo más común de «no puedo entrar»—. Más **el rótulo del mes convertido en selector** con una flecha a cada lado, **la Bandeja del teléfono** resolviendo lo mismo que la del iPad («Devolver al tesorero» deslizando, en ámbar y no en rojo porque devolver no destruye nada; y fuera el «Aprobar» que no hacía nada), y dos arreglos de aparato. Arnés: **1116** ✓ / 0 ✗. **Ninguna colisión esta vez**: `1.3.5` no lo usa ninguna dependencia en ninguno de los dos lockfiles — las tres seguidas anteriores (`proc-macro-crate`, `bitflags`, `tiny-invariant`) fueron mala suerte de números corrientes, no una regla. **Y la etiqueta se puso ANTES de compilar**, por primera vez sin que hiciera falta pedirlo: `v1.3.5` sobre `92ef601`, empujada desde el Mac. Eso es lo que cierra el defecto que la 1.2.10, la 1.2.11 y la 1.3.0 enseñaron tres veces —commits colándose después del número, y «la 1.3.0» quedando como un rango difuso en vez de un punto—. Ahora `git checkout v1.3.5` devuelve exactamente lo que hay en TestFlight |
| **ninguna preparada** | **La 1.3.6 no existe todavía, y es lo correcto.** Desde el bump a la 1.3.5 (`92ef601`) no ha entrado ni un commit de producto, y las tres ramas de diseño están en ese mismo punto. **Tercera vez que el número NO va por delante del trabajo** — antes la 1.3.1 y la 1.3.5. Lo que hay en marcha es el **Registro de la iglesia**, encargado a los tres chats de diseño el 29 de agosto (`docs/encargo-registro.md`): es la única pantalla sin diseño en ninguna de las cuatro capas. Cuando vuelva de los tres aparatos, eso será la 1.3.6 |
...
>
> ⚠️ **La colisión de prefijo dejó de ser hipotética el 27 de agosto de 2026.**
> Al preparar la 1.3.2, `version = "1.3.1"` salía **DOS veces** en
> `Cargo.lock`: la línea 3852, que es Tamio, y la **2708, que es
> `proc-macro-crate`** — una dependencia que casualmente va en esa misma
> versión. Un `sed s/1.3.1/1.3.2/` habría subido de versión un paquete de
> terceros en el lockfile, y eso no falla al compilar: falla al resolver, o
> peor, no falla y trae otro paquete.
>
> **Tres de tres, y la tercera cambió de archivo.** Con la 1.3.4, `1.3.3`
> salía tres veces: las dos de Tamio y una de **`tiny-invariant`**, esta vez en
> `package-lock.json`. Hasta entonces las colisiones habían salido todas en
> `Cargo.lock` y se podía pensar que era cosa de Rust. No lo es: **los dos
> lockfiles tienen la trampa**, y el de npm más todavía, porque una app de
> React arrastra cientos de paquetes pequeños con versiones bajas.
>
> **Y pasó OTRA VEZ al día siguiente**, con la 1.3.3: `1.3.2` salía dos veces,
> la 3852 de Tamio y la **257 de `bitflags`**, que va en esa versión desde hace
> años. Dos de dos. Deja de ser una casualidad y pasa a ser lo normal: en un
> lockfile de casi cuatro mil líneas con cientos de paquetes, cualquier número
> de versión corriente —1.3.1, 1.3.2, 2.0.0— choca con alguno.
>
> Hasta ese día la regla de **contar antes de tocar** sólo había cazado
> prefijos (`cc` en 1.2.67 pareciéndose a 1.2.6). Ahora tiene un caso real de
> colisión exacta. **Nunca buscar-y-reemplazar en un lockfile.** Se cuenta, se
> ancla en `name = "tesoreria"`, y se cambia POR NÚMERO DE LÍNEA.
...
>
> **CORRECCIÓN, 29 ago 2026 — el paso ya está hecho y yo lo daba por pendiente.**
> Las seis etiquetas están arriba: `v1.2.12`, `v1.3.0`, `v1.3.1`, `v1.3.2`,
> `v1.3.3` y `v1.3.4`, y cada una apunta al commit de SU número, no a uno
> posterior. Iván las subió sobre la marcha. Yo seguí escribiendo aquí que
> faltaban porque miraba mi propio 403 y no el remoto: el error de siempre —dar
> por hecho el estado del mundo desde lo que a mí me falla—. Comprobarlo cuesta
> un mandato: `git ls-remote --tags origin`.
